...
function a(){
    getArray().then((data) => { //then箭頭函式
        let EducationCount ={} //宣告學歷陣列
        data.forEach((i) => { // forEach 代表 data 取出的資料為 i
        const edu = i.education //宣告 edu = data 中的 education 資料
        if (EducationCount[edu] === undefined){ //如果陣列中的 education 資料還沒有被賦值
...
function b(){
    ...
    const result = Object.keys(areaCount).map((area) => { //宣告 result keys 為 areaCount 重新輸出一個 area 陣列
...
function c(){
    return getArray().then((data) => {
        ...
        let count = 0
        data.forEach((i) => {
            if (i.age === '26~30 歲') { //判斷年齡 如果為26-30歲 記數加一 加薪水滿意度分數加入score變數
                count +=1 
...
function d(){
    return getArray().then((data) => {
        let industry_message = { //新增一個有紀錄 有寫跟沒寫的陣列
            '有寫': 0,
            '沒寫': 0
        };
        data.forEach((i) => { 
...
function e(){
    getArray().then((data) => {
        ...
        if (industryCount[industry] === undefined){ //判斷陣列中有沒有沒賦值過的陣列
           industryCount[industry] = {}; //沒有的話新增一個
        }
        if (industryCount[industry][workplace] === undefined){ // 判斷工作區域有沒有出現過
            industryCount[industry][workplace] = 1;} //沒有的話新增一個
        else{
...
function f() {
  getArray().then((data) => {
    ...
    data.forEach((i) => {
      ...
      if (industryCount[industry] === undefined) { //判端是不是沒出現過的產業別
        industryCount[industry] = { // 生成新產業別的模板
          "男性人數:": 0,
          "女性人數:": 0,
          男性產業滿意度: 0,
          女性產業滿意度: 0,
...
function g() {
  getArray().then((data) => {
    ...
    data.forEach((i) => {
      ...
      if (jobTenure[tenure] === undefined) { //判端是不是沒出現過的年資
        jobTenure[tenure] = { //新增沒出現過的年資模板
          實體辦公室的平均薪水滿意度: 0,
          遠端工作的平均薪水滿意度: 0,
          混合制的平均薪水滿意度: 0,
      ...
      if (workPlace === "遠端工作") { //判斷工作區域
      ...
      } else if (workPlace === "實體辦公室") { //判斷工作區域
        ...
        jobTenure[tenure]["實體辦公室的平均薪水滿意度"] =
          jobTenure[tenure]["實體辦公室的總和薪水滿意度"] / jobTenure[tenure]["實體辦公室人數"]; //取平均
      } else { //判斷工作區域
        ...
        jobTenure[tenure]["混合制的平均薪水滿意度"] = 
          jobTenure[tenure]["混合制的總和薪水滿意度"] / jobTenure[tenure]["混合制人數"]; //取平均
      }
    });
